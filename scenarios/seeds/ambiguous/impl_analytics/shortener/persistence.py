...
The cache and analytics queue are derived views; this module owns durability,
unique-code allocation under concurrency, idempotent creates, and optimistic
concurrency for destination updates. Migrations apply in filename order and
are tracked in ``schema_migrations`` so a populated baseline upgrades in place.
"""
...
class Database:
    ...
    def create_link(self, url: str, expires_at: str | None = None,
        ...
        """Insert a new link. Returns (link, created); created is False when an
        idempotency key replays an earlier create."""
        with self.lock, self.conn:
            if idempotency_key:
    ...
    # -- analytics ------------------------------------------------------------
    def record_clicks(self, batch: list[tuple[str, str]]) -> None:
        """Persist a batch of (code, ts) events and refresh aggregates."""
        if not batch:
            return
        with self.lock, self.conn:
            self.conn.executemany(
                "INSERT INTO click_events (code, ts) VALUES (?, ?)", batch)
            for code, ts in batch:
                self.conn.execute(
                    "INSERT INTO click_totals (code, clicks, last_click) "
                    "VALUES (?, 1, ?) "
                    "ON CONFLICT(code) DO UPDATE SET "
                    "clicks = clicks + 1, last_click = excluded.last_click",
                    (code, ts))

    def click_stats(self, code: str) -> dict:
        with self.lock:
            row = self.conn.execute(
                "SELECT clicks, last_click FROM click_totals WHERE code = ?",
                (code,)).fetchone()
        if row is None:
